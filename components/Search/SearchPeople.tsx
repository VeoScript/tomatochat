...
const SearchPeople = () => {
  ...
  return (
    <React.Fragment>
      <span className="inline-flex items-center w-[20rem] px-3 py-2 z-20 space-x-2 rounded-lg text-zinc-800 dark:text-zinc-400 bg-white dark:bg-[#201A2C] border border-zinc-300 dark:border-transparent focus-within:border-purple-600 dark:focus-within:border-purple-600">
        <input
          type="text"
      ...
      {isDisplay && (
        <React.Fragment>
          ...
          <div className="absolute top-16 z-20 w-[20rem]">
            <div className="flex w-full h-full max-h-[20rem] -ml-[1.2rem] overflow-y-auto shadow-sm rounded-md bg-zinc-50 dark:bg-gradient-to-br dark:from-[#1B1325] dark:via-[#12111B] dark:to-[#18132A] focus:outline-none">
              <div className="flex flex-col w-full divide-y divide-zinc-100 dark:divide-[#1F1E35]">
                ...
                {searchResults?.map(( user: {id: string, name: string, image: string, username: string}, i: number ) => (
                  ...
                  >
                    <a
                      className="inline-flex items-center space-x-2 p-2 font-light text-sm text-left cursor-pointer transition ease-in-out duration-200 hover:bg-zinc-100 dark:hover:bg-[#1F1E35]"
                      onClick={() => {
                        setIsDisplay(false)
                        setSearchTerm('')
                      }}
