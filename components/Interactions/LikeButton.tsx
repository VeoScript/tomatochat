...
const LikeButton: React.FC<IProps> = ({ user, post }) => {
  ...
  return (
    <button
      ...
      {like ? (
          <RiHeart3Fill className="w-6 h-6 text-red-600 hover:text-red-600 focus:text-neutral-500" />
        ) : (
          <RiHeart3Fill className="w-6 h-6 text-neutral-500 hover:text-red-600 focus:text-red-600" />
        )
      }
    </button>
  )
