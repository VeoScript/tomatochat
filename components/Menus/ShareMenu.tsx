...
const ShareMenu: React.FC<IProps> = ({ children, title, user, post }) => {
  ...
  const shareOnFB = () => {
    var url = `https://www.facebook.com/sharer/sharer.php?u=${process.env.VERCEL_BASE_URL}post/${post.id}&t=${post.description}`
    window.open(url, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=900')
    return false
  }

  const shareOnTwitter = () => {
    var url = `https://twitter.com/intent/tweet?url=${process.env.VERCEL_BASE_URL}post/${post.id}&text=${post.description}`
    window.open(url, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=900')
    return false
  }
