...
import { RiSearchLine, RiSpyFill } from 'react-icons/ri'
  ...
  return (
    <div className="flex flex-col w-full h-full overflow-hidden border-x border-[#1F1836]">
      ...
      <div className="flex flex-col w-full h-full p-3 overflow-y-scroll scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-transparent">
        ...
        {isError && (
          <div className="inline-flex items-center justify-center w-full h-full space-x-1 text-sm">
          ...
          <React.Fragment>
            {rooms && rooms.pages.map((page: any) => (
              <React.Fragment key={page.nextId ?? 'lastPage'}>
                {page.rooms.map((room: { photo: string, name: string, description: string }, i: number) => (
                  <div
                    key={i}
